...
if (isset($_GET['bulan'],$_GET['tahun'])) {
  $bln = $_GET['bulan'];
  $tahun = $_GET['tahun'];
  $resultt = $koneksi->query("SELECT * FROM laporan");
}else {
  $result = $koneksi->query("SELECT * FROM laporan_a WHERE jenis_meter='listrik' ORDER BY id_laporan DESC");
}
?>
<html>
<head>
  ...
  <div class="content-wrapper">
    <section class="content-header">
      <div class="container-fluid">
        <div class="row mb-2">
          <div class="col-sm-6">
            <h1>Laporan Transaksi Admin</h1>
          </div>
            <div class="col-sm-6">
              <ol class="breadcrumb float-sm-right">
                <li class="breadcrumb-item"><a href="#">Home</a></li>
                <li class="breadcrumb-item active">Laporan Transaksi Listrik</li>
              </ol>
           </div>
         </div>
       </div>
    ...
    <section class="content">
      <div class="row">
        ...
        <br>
        <br>
        <form id="sub" action="?laporan=delete-x" method="POST" >
        ...
        <div class="table-responsive">
          <table id="example1" class="table table-striped">
            ...
            <tbody>
              <?php
                $no = 1;
                $resultt = $koneksi->query("SELECT * FROM laporan_a WHERE jenis_meter='listrik' ORDER BY id_laporan DESC");
              ...
              <tr>
                ...
                <td>
                  <div class="d-flex">
                    <a class="btn btn-success btn-sm text-nowrap" href="?laporan=export&id=<?= $data['id_laporan']?>"><i class='fas fa-file-export'></i></a>&nbsp
                    
                  </div>
                </td>
                
                <td><input type="checkbox" class="check-item" name="pilih[]" id="check-all" value="<?= $data['id_laporan']; ?>"></td>
              </tr>
                  </div>
                  <?php } ?>
              </tbody>
